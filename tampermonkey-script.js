...
(function () {
  ...
  function init() {
    ...
    for (let timeEntry of timeEntries) {
      ...
      const timeElement = timeEntry.querySelector('.ui-li-aside');
      const timeText = timeElement ? timeElement.innerHTML.match(/\d{2}:\d{2}-\d{2}:\d{2}/) : "";
      const hourMatch = timeElement ? timeElement.innerHTML.match(/(\d+\.\d{2})/) : "";

      console.log("  time text:", timeText);
      console.log("  hour match:", hourMatch);

      if (timeText && hourMatch) {
        workedHours += parseFloat(hourMatch[1]);
      }

      if (timeElement && timeElement.textContent.includes('Kesken')) {
